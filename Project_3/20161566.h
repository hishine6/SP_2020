...
#define MAX_LOCCTR_NUM 1000	//LOCCTR의 최대 갯수
#define REGISTER_NUM 9	//레지스터의 총 갯수
...
int Loader_Run_start_address;	//load나 run의 시작 주소를 저장
int running;	//현재 breakpoint로 인해 중단된 run이 있는지 없는지

int R[REGISTER_NUM];	//run에서 사용되는 레지스터들
/*-----------------------------
0: a
1: x
2: l
...
6: PC
7: CC(status word)
8: 가장 최근에 loading된 길이
-------------------------------*/
int breakpoint[MAX_LOCCTR_NUM];	//Break point를 저장하기 위한 배열
int breakpoint_num = 0;	//breakpoint의 갯수
...
typedef struct external_node {
	char label[MAX_TOKEN_SIZE];	//label이름을 저장한다.
	int loc;		//해당 주소를 저장한다.
	int length;		//이 값이 -1이라면 제어섹션이 아닌, external deffinition이다. -1이 아니라면 제어섹션의 길이를 저장한다.
	struct external_node* next;
}external_node;
external_node* estab_head;	//ESTABLE의 머리
...
/*----------------------------------------------------
Progaddr명령어를 실행한다
run이나 loader의 시작 주소를 지정한다.
----------------------------------------------------*/

void Loader();
/*----------------------------------------------------
Loader 명령어를 실행한다.
Pass1과 Pass2로 나뉘며 앞에서 만든 메모리에 코드를 오린다.
----------------------------------------------------*/

int Loader_Pass1();
/*----------------------------------------------------
Loader pass 1을 실행한다.
-->Definition부분을 읽어서 ESTABLE을 만든다
-->프로그램의 총 길이를 저장하고 반환한다.
----------------------------------------------------*/
...
Loader pass 2를 실행한다.
-->R, T, M부분을 실행한다.
-->R로 01, 02, 03들의 실제 주소를 저장한다.
-->T로 memory에 주어진 object code를 올린다.
-->M으로 각 memory를 수정한다.
----------------------------------------------------*/

int Add_ESTAB(char tlabel[], int tloc, int tlength);
/*----------------------------------------------------
EXTAB에 변수를 추가한다.
-->linked list형태로 연결한다.
-->만약 이미 있는 LABEL이라면 그 위치를 반환한다.
----------------------------------------------------*/

void BP();
/*----------------------------------------------------
breakpoint를 출력, 추가, 삭제하는 명령어이다
인자가 평범한 LOCCTR이면 추가한다.
인자가 ClEAR이면 breakpoint를 모두 삭제한다
인자가 없다면 저장된 breakpoint를 모두 출력한다.
----------------------------------------------------*/

int BP_Match(int target);
/*----------------------------------------------------
BP중에 target의 주소가 있는지 아닌지 확인한다.
----------------------------------------------------*/

void Run();
/*----------------------------------------------------
run명령어를 실행한다.
-->Loader_Run_start_address의 주소부터 run을 한다.
-->해당하는 메모리를 해석하여 opcode, nixbpe, displya 또는 target을 계산한다.
-->bp에 해당하는 pc의 값을 만나면 종료한다.
-->프로그램의 끝에 도달하면 종료한다.
----------------------------------------------------*/