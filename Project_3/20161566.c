...
int main() {
	...
	estab_head = (external_node*)malloc(sizeof(external_node));
	estab_head->next = NULL;
	...
	Loader_Run_start_address = 0;	//load와 run에 사용되는 시작 주소를 0으로 설정한다.
...
int Check_Command() {	//Check_Command함수, 명령어에 따라서 유효함을 판단한다.																				
	...
	if (strcmp(temp_value[0], "loader") != 0) {
	...
	else if (strcmp(temp_value[0], "progaddr") == 0) {
		if (temp_value_num != 2) {
			printf("ERROR: Right command Wrong use\n");
			return 0;
		}
	...
	else if (strcmp(temp_value[0], "bp") == 0) {
		if (temp_value_num == 1) {	//break point를 모두 출력한다.
			value_num = 0;
			return 15;
		}
		else if (temp_value_num == 2) { //break point를 추가하거나 모두 삭제한다.
			if (strcmp(temp_value[1], "clear") == 0 || Check_Address(temp_value[1]) == 1) {
				strcpy(value[0], temp_value[1]);
				value_num = 1;
				return 15;
			}
	...
	else if (strcmp(temp_value[0], "loader") == 0) {
		if (temp_value_num == 2) {
			value_num = 1;
			strcpy(value[0], temp_value[1]);
			return 16;
	...
	else if (strcmp(temp_value[0], "run") == 0) {
		if (temp_value_num == 1) {
			return 17;
		}
		else {
...
void Assemble() {
	...
	int location_save[MAX_LOCCTR_NUM];	// LOCCTR을 저장하여, PC의 위치, 즉 다음 LOCCTR을 구한다.
	...
	int modification[MAX_LOCCTR_NUM];	//obj파일의 modification에서 사용된다.
...
void Progaddr() {	//Progaddr 명령어를 수행한다.
	Loader_Run_start_address = (int)strtol(value[0], NULL, 16);	//loader와 run의 시작 주소를 설정한다.
	Add_History();
}

void Loader() {	//loader 명령어를 수행한다.

	external_node* temp;	//미리 존재하는 estable이 있다면 이를 삭제한다.
	while (estab_head != NULL) {
		temp = estab_head;
		estab_head = estab_head->next;
		free(temp);
	...
	estab_head = (external_node*)malloc(sizeof(external_node));	//새로운 estable을 만든다.
	estab_head->next = NULL;

	int LENGTH = Loader_Pass1();	//loader의 pass1을 실행하고 프로그램의 길이를 LENGHT에 저장한다.
	if (LENGTH == -1)	//반환 값이 -1이라는 것은 loading이 제대로 수행되지 않았다는 뜻이다.
		return;
	if (Loader_Pass2() == 0)	//loader pass2를 수행하고 오류가 발생하면 종료한다.
		return;


	temp = estab_head->next;

	printf("control symbol address length\nsection name\n-----------------------------\n");	//load map을 출력한다.
	while (temp != NULL) {
		if (temp->length == -1) {	//만약 synmbol name이면,
			printf("\t%s\t%04X\n", temp->label, temp->loc);
		}
		else {	//만약 Control section이면,
			printf("%s\t\t%04X\t%04X\n", temp->label, temp->loc, temp->length);
	...
	while (estab_head != NULL) {	//loader가 종료되면 estable을 제거한다.
		temp = estab_head;
		estab_head = estab_head->next;
		free(temp);
		temp = NULL;
	}
	for (int i = 0; i < REGISTER_NUM; i++)	//레지스터들을 0으로 초기화 한다.
		R[i] = 0;
	R[8] = LENGTH;	//load된 프로그램의 길이를 R[8]이라는 변수에 저장한다.
	R[2] = LENGTH; //L레지스터에 프로그램의 길이를 저장한다.
	R[6] = Loader_Run_start_address;	//PC에 시작 주소를 넣는다.
	Add_History();
}

int Loader_Pass1() {
	int CSLTH = 0;	//현재 control section의 길이
	int CSADDR = Loader_Run_start_address;	//현재 control sectino의 시작 주소
	int LENGTH = Loader_Run_start_address;	//전체 프로그램의 길이 계산을 위해 사용
	int cycle = 0;
	char token[10][MAX_TOKEN_SIZE];
	int token_num = 0;
	int size;
	int record_style = 0;
	external_node* estab_last;
	FILE* fp;

	while ((cycle) < value_num) {	//총 obj의 파일의 갯수만큼 반복
		fp = fopen(value[cycle++], "r");
		if (fp == NULL) {
			printf("ERROR: No such File Exists\n");
			return -1;
		}
		char file_input[100];
		while (fgets(file_input, sizeof(file_input), fp) != NULL) {	//obj파일의 한줄씩 읽는다.
			size = strlen(file_input);	//각줄의 개행을 제외하고 저장한다.
			if (file_input[size - 2] == 13)	//만약 '\n'이전에 CR이 있다면
				size -= 2;
			else
			...
			if (record_style == 1) {	//record가 Head인 경우
				CSLTH = (int)strtol(token[1], NULL, 16);	//이 control section의 길이를 저장한다.
				if (Add_ESTAB(token[0], CSADDR, CSLTH) != -1) {	//estable에 저장한다.
					printf("ERROR: duplicate external symbol\n");	//만약 이미 존재한다면 오류를 출력
					return -1;
				}
			}
			else if (record_style == 2) {	//record가 Definition인 경우
				for (int i = 0; i < token_num - 1; i++) {
					char temp_loc[7];
					int loc;
					char label[MAX_TOKEN_SIZE];
					for (int j = 0; j < 6; j++)
						temp_loc[j] = token[i + 1][j];
					temp_loc[6] = '\0';
					loc = (int)strtol(temp_loc, NULL, 16);
					if (i != 0) {
						int temp_size = strlen(token[i]);
						for (int j = 0; j < temp_size - 5; j++)
							label[j] = token[i][j + 6];
					}
					else
						strcpy(label, token[i]);
					if (Add_ESTAB(label, CSADDR + loc, -1) != -1) {	//Definition의 label과 위치를 읽어와서 estable에 저장한다.
						printf("ERROR: duplicate external symbol\n");
						return -1;
					}
				}
		...
		CSADDR += CSLTH;	//다음 control section의 시작 주소를 저장한다.
		fclose(fp);
	}
	return CSADDR - LENGTH;	//프로그램의 총 길이를 반환한다.
}

int Loader_Pass2() {
	int CSADDR = Loader_Run_start_address;	//control section의 시작 주소
	int CSLTH = 0;	//control section의 길이
	int cycle = 0;
	int size = 0;
	int External_Reference[20];	//external reference로 선언한 label들을 저장한다.(예: +01, +02 등등)
	FILE* fp;
	memset(External_Reference, 0, sizeof(External_Reference));

	while ((cycle) < value_num) {	//obj파일 하나씩 돌면서,
		fp = fopen(value[cycle++], "r");
		if (fp == NULL) {
			printf("ERROR: No such File Exists\n");
			return 0;
		...
		while (fgets(file_input, sizeof(file_input), fp) != NULL) {	//obj파일의 한줄씩 읽는다.
			...
			if (file_input[0] == 'E')
				break;
			else if (file_input[0] == 'H' || file_input[0] == 'R') {	//head 또는 Reference 인 경우
				char token[10][MAX_TOKEN_SIZE];
				memset(token, 0, sizeof(token));	//토큰의 초기화
				int token_num = 0;
				//한 줄을 공백을 기준으로 token으로 나눔
				...
				//한 줄을 공백을 기준으로 token으로 나눔 완료
				if (file_input[0] == 'H')
					CSLTH = (int)strtol(token[1], NULL, 16);	//control section의 길이를 저장한다.
				else {
					External_Reference[1] = CSADDR;	//external reference 01은 control section의 시작 주소로 설정한다.
					for (int i = 0; i < token_num; i++) {	//02, 03등 reference에 해당하는 주소를 External reference에 저장한다.
						char temp[3];
						char label[MAX_TOKEN_SIZE];
						int temp_size = strlen(token[i]);
						temp[0] = token[i][0];
						temp[1] = token[i][1];
						temp[2] = '\0';
						int index = (int)strtol(temp, NULL, 10);	//index(예: 01, 02, 03)을 저장
						for (int j = 2; j < temp_size; j++)
							label[j - 2] = token[i][j];
						label[temp_size - 2] = '\0';	//label(예: ENDA, ENDB등을 저장)
						int temp_estab = Add_ESTAB(label, 0, 0);	//label에 해당하는 주소 저장
						if (temp_estab == -1) {
							printf("ERROR: Wrong ESTAB\n");
							return 0;
						}
						External_Reference[index] = temp_estab;	//해당 index에 주소 저장
					}
				}
			}
			else if (file_input[0] == 'T' || file_input[0] == 'M') {	//Text, Modification 명령어인 경우
				char temp[7];	//
				int address;	//저장을 시작할 주소를 저장한다.
				int length;		//총 길이를 저장한다.
				int change;
				int ER;
				for (int i = 0; i < 6; i++)	//앞의 T, M을 없앤다.
					temp[i] = file_input[i + 1];
				temp[6] = '\0';
				address = (int)strtol(temp, NULL, 16);	//앞의 6자리 수를 주소로 저장한다.
				temp[0] = file_input[7];
				temp[1] = file_input[8];
				temp[2] = '\0';
				length = (int)strtol(temp, NULL, 16);	//그 다음 두 수를 길이로 저장한다.
				if (file_input[0] == 'T') {	//Text 라면
					for (int i = 9; i < 2 * length + 9; i += 2) {	//2개씩 읽어서
						temp[0] = file_input[i];
						temp[1] = file_input[i + 1];
						temp[2] = '\0';
						change = (int)strtol(temp, NULL, 16);	//16진수로 변환한 뒤
						memory[address + CSADDR] = (char)change;	//메모리 하나에 1byte로 저장한다.
						address++;
					}
				}
				else {	//Modification이라면
					int plus_minus = 0;	//더하는지 빼는지
					if (file_input[9] == '-')
						plus_minus = 1;
					temp[0] = file_input[10];
					temp[1] = file_input[11];
					temp[2] = '\0';
					ER = External_Reference[(int)strtol(temp, NULL, 10)];	//숫자에 맞는 external reference 주소를 가져온다.
					int a = (int)memory[address + CSADDR];	//address+CASDDR부터 3바이트 만큼 가져온다.
					int b = (int)memory[address + CSADDR + 1];
					int c = (int)memory[address + CSADDR + 2];
					sprintf(temp, "%02X%02X%02X", a, b, c);	//가져온 3바이트를 하나의 16진수로 합친다.(문자열)
					int d = (int)strtol(temp, NULL, 16);	//저장한 16진수
					if (length == 5)	//2+half byte만큼 조정하는 경우
						d = d & 0XFFFFF;
					if (plus_minus == 1)
						d -= ER;
					else
						d += ER;
					if (length == 5)	//길이가 5이면 원래 있던 처음 half byte는 그대로, 수정된 half byte는 뒤에 넣어준다.
						memory[address + CSADDR] = ((d & 0xF0000) >> 16) + (a & 0xF0);
					else	//다시 메모리에 넣는 부분.
						memory[address + CSADDR] = (d & 0xFF0000) >> 16;
					memory[address + CSADDR + 1] = (d & 0xFF00) >> 8;
					memory[address + CSADDR + 2] = d & 0xFF;
				}
			}
			else
				continue;
		...
		CSADDR += CSLTH;	//다음 control section의 주소를 가리킨다.
		fclose(fp);
	}
	return 1;
}

int Add_ESTAB(char tlabel[], int tloc, int tlength) {	//ESTABLE을 추가하고, 탐색하기 위한 함수
	external_node* temp = estab_head;
	while (temp->next != NULL) {	//앞에서부터 확인을 하면서 같은 것이 있다면 그 위치를 반환한다.
		if (strcmp(temp->label, tlabel) == 0) {
			return temp->loc;
		}
		temp = temp->next;
	}
	if (strcmp(temp->label, tlabel) == 0)
		return temp->loc;
	external_node* new = (external_node*)malloc(sizeof(external_node));	//마지막위치에 parameter정보를 바탕으로 추가한다.
	strcpy(new->label, tlabel);
	new->loc = tloc;
	new->length = tlength;
	new->next = NULL;
	temp->next = new;
...
void Run() {
	if (running == 0) {	//이미 실행 중인 run이 없다면
		for (int i = 0; i < REGISTER_NUM; i++) {	//레지스터들을 0으로 초기화한다.
			if (i == 8)		//R[8]은 레지스터가 아닌 프로그램의 길이를 저장하고 있다.(초기화하지 않는다)
				continue;
			R[i] = 0;
		}
		R[6] = Loader_Run_start_address;	//PC에 시작 주소를 넣는다.
		R[2] = R[8];	//L에 가장 최근에 load된 길이를 저장한다.
		running = 1;	//run이 실행 중임을 표시한다.
	}
	int PC = R[6];	//PC라는 지역변수에 PC레지스터의 값을 저장한다.
	if (R[2] == 0) {	//loading된 프로그램이 존재하지 않는 경우
		printf("ERROR: No Program Loaded, Linked\n");
		running = 0;
		return;
	}
	while (PC != R[8]) {	//PC가 program length에 도달할 때까지
		int target = (int)memory[PC++];
		int opcode = target & 0xFC;	//처음 byte를 읽어서 opcode를 해석한다.
		if (opcode == 0xB4 || opcode == 0xA0 || opcode == 0xB8) {	//format 2의 경우
			target = (int)memory[PC++];
			int first_register = target & 0xF0;	//처음 레지스터의 값을 저장한다.
			first_register = first_register >> 4;
			int second_register = target & 0x0F;	//두번째 레지스터의 값을 저장한다.
			if (opcode == 0xA0) {	//COMPR
				if (R[first_register] > R[second_register])
					R[7] = '>';
				else if (R[first_register] == R[second_register])
					R[7] = '=';
				else
					R[7] = '<';
			}
				...
				R[1]++;
				if (R[1] > R[first_register])
					R[7] = '>';
				else if (R[1] == R[first_register])
					R[7] = '=';
				else
					R[7] = '<';
			}
		}
		else {	//format 3또는 4의 경우
			int n = (target & 0x2) >> 1;
			int i = (target & 0x1);
			int display = 0;	//target address를 구하기 위한 display또는 address
			target = (int)memory[PC++];
			int x = (target & 0x80) >> 7;
			int b = (target & 0x40) >> 6;
			int p = (target & 0x20) >> 5;
			...
			if (e == 1) {	//format 4인 경우
				...
				display *= (16 * 16);
				target = (int)memory[PC++];
				display += target;	//display에 target address가 저장된다.
			}
			else {	//format 3인 경우
				display += (target & 0xF);
				display *= (16 * 16);
				target = (int)memory[PC++];
				display += target;	//display 를 구한다.

				if (display & 1 << 11) {	//display 가 음수인 경우
					display = ~display + 1;
					display = display & ((1 << 12) - 1);
					display *= -1;
				}

				if (p == 1 && b == 0)	//PC relative를 사용하는 경우
					display += PC;
				else if (p == 0 && b == 1)	//BASE relative를 사용하는 경우
					display += R[3];
			}
			if (n == 1 && i == 0) {	//indirect addressing인 경우
				char temp[7];	//한번 더 주소를 계산하여 display를 계산한다.
				int a = (int)memory[display];
				int b = (int)memory[display + 1];
				int c = (int)memory[display + 2];
				sprintf(temp, "%02X%02X%02X", a, b, c);
				int d = (int)strtol(temp, NULL, 16);
				display = d;
			}

			if (x == 1) {	//X레지스터를 사용하는 경우
				display += R[1];	//X레지스터만큼 display에 더해준다.
			}
			...
			if (opcode == 0x00) {	//LDA
				if (n == 0 && i == 1)
					...
					char temp[7];	//display부터 3byte만큼의 메모리를 읽어와서 A에 저장한다.
					int a = (int)memory[display];
					int b = (int)memory[display + 1];
					int c = (int)memory[display + 2];
					sprintf(temp, "%02X%02X%02X", a, b, c);
					R[0] = (int)strtol(temp, NULL, 16);
				}
			}
			else if (opcode == 0x10) { //STX
				int temp = R[1];	//X레지스터의 값을 display주소의 3byte만큼 할당한다.
				int a = 0xFF;
				memory[display + 2] = a & temp;
				temp = temp >> 8;
				memory[display + 1] = a & temp;
				temp = temp >> 8;
				memory[display] = a & temp;
			}
			else if (opcode == 0x14) {	//STL
				int temp = R[2];	//L레지스터의 값을 display주소의 3byte만큼 할당한다.
				int a = 0xFF;
				memory[display + 2] = a & temp;
				temp = temp >> 8;
				memory[display + 1] = a & temp;
				...
				if (n == 0 && i == 1)
					...
					char temp[7];	//display에서부터 3byte만큼 읽어와서 A와 비교한다.
					int a = (int)memory[display];
					int b = (int)memory[display + 1];
					int c = (int)memory[display + 2];
					sprintf(temp, "%02X%02X%02X", a, b, c);
					compare = (int)strtol(temp, NULL, 16);
				}
				if (R[0] > compare)
					R[7] = '>';	//비교 결과는 R[7]에 저장된다.
				else if (R[0] == compare)
					R[7] = '=';
				else
					R[7] = '<';
				...
				R[0] = 0xFF & memory[display];
			}
			else if (opcode == 0x54) {	//STCH
				memory[display] = R[0] & 0xFF;
			}
			else if (opcode == 0x68) {	//LDB
				if (n == 0 && i == 1)
					R[3] = display;
				else {
					char temp[7];	//display부터 3byte만큼의 메모리를 읽어와서 B에 저장한다.
					int a = (int)memory[display];
					int b = (int)memory[display + 1];
					int c = (int)memory[display + 2];
					sprintf(temp, "%02X%02X%02X", a, b, c);
					R[3] = (int)strtol(temp, NULL, 16);
				}
			}
				...
				if (n == 0 && i == 1)
					...
					char temp[7];	//display부터 3byte만큼의 메모리를 읽어와서 L에 저장한다.
					int a = (int)memory[display];
					int b = (int)memory[display + 1];
					int c = (int)memory[display + 2];
					sprintf(temp, "%02X%02X%02X", a, b, c);
					R[5] = (int)strtol(temp, NULL, 16);
				}
			}
			else if (opcode == 0x0C) {	//STA
				int temp = R[0];	//A레지스터의 값을 display주소의 3byte만큼 할당한다.	
				int a = 0xFF;
				memory[display + 2] = a & temp;
				temp = temp >> 8;
				memory[display + 1] = a & temp;
		...
		if (BP_Match(PC)) {	//BP를 만나는 경우
			R[6] = PC;	//PC레지스터에 PC의 값을 저장한다.
			printf("A : %06X  X : %06X\nL : %06X PC : %06X\nB : %06X  S : %06X\nT : %06X\n", R[0], R[1], R[2], PC, R[3], R[4], R[5]);
	...
	running = 0;	//프로그램이 끝나면 running은 0이 된다.
	printf("A : %06X  X : %06X\nL : %06X PC : %06X\nB : %06X  S : %06X\nT : %06X\n", R[0], R[1], R[2], PC, R[3], R[4], R[5]);
	printf("\t\tEnd Program\n");
	memset(breakpoint, -1, sizeof(breakpoint));	//break point를 초기화 한다.
	breakpoint_num = 0;
	Add_History();
}

void BP() { // bp 명령어 수행
	if (value_num == 0) {	//저장된 bp의 값을 출력한다.
		printf("\t\tbreakpoint\n\t\t----------\n");
		for (int i = 0; i < breakpoint_num; i++)
			printf("\t\t%X\n", breakpoint[i]);
	}
	else {	//bp의 값을 초기화 한다.
		if (strcmp(value[0], "clear") == 0) {
			memset(breakpoint, -1, sizeof(breakpoint));
			breakpoint_num = 0;
		}
		else {	//bp를 추가한다.
			int target = (int)strtol(value[0], NULL, 16); //값을 breakpoint배열 뒤에 추가한다
			breakpoint[breakpoint_num++] = target;
...
int BP_Match(int target) { //해당 주소가 bp인지 확인한다.
	for (int i = 0; i < breakpoint_num; i++) {	//breakpoint를 돌면서 bp인지 확인한다.
		if (breakpoint[i] == target) {
			return 1;
		}
	}
	return 0;
